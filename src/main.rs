...
async fn main() {
    ...
    let records: Vec<DatabaseTable> = client.fetch_table("Movie Database Table").await.unwrap_or(vec![]);
    println!("Fetched Records:");
    for record in records {
        println!("ID: {}, Title: {}, Actors: {}, Released: {}, Summary: {}", record.id, record.title, record.actors, record.released, record.summary);
    }
